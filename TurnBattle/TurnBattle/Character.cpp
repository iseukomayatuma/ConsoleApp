...
// プレーヤ表示を行う
void IndicatePlayer(Character* ch)    // playerのポインタを受け取ってる
{
	if (ch->hp < ch->maxHp / 5) printf(EscRED);
	else if (ch->hp < ch->maxHp / 3) printf(EscYELLOW);
	else if (ch->hp == ch->maxHp) printf(EscGREEN);
	printf("%s\nH P ; %d／%d　M P : %d／%d", ch->name, ch->hp, ch->maxHp, ch->mp, ch->maxMp);
	printf(EscDEFAULT);
}
// エネミー表示を行う
void IndicateEnemy(Character* ch)
{
	if (ch->hp != 0)
	{
		...
		printf("%s (H P : %d／%d)", ch->aa, ch->hp, ch->maxHp);
		printf(EscDEFAULT);
